...
*   **Backend (PocketBase):** The backend can be hosted on any platform that supports Go executables and a persistent filesystem. Recommended free options that do not require a credit card include **Okteto** and **Render**.
...
### Note:
Encryption For Client Side and Data at Rest shall be added in next iteration.
...
In the App.jsx,
If you Use For Web App, Modify The Code
`import { BrowserRouter } from 'react-router-dom';` -> Include This Line.
If you Use For Desktop App, Modify The Code
`import { BrowserRouter } from 'react-router-dom';` -> Delete This Line.
